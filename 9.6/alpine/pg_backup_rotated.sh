...
function perform_backups()
{
	...
	###################################
	### VERIFY INCOMPLETE BACKUPS ####
	###################################

	if [ -d "$FINAL_BACKUP_DIR" -a ! -a "$FINAL_BACKUP_DIR"__incomplete__ ]
	then
		echo "Backup already completed. Exiting!!!!"
		exit 0;
	fi;
	...
	touch "$FINAL_BACKUP_DIR"__incomplete__
	...
	# removing file which we touched to detect empty backups
	rm -f "$FINAL_BACKUP_DIR"__incomplete__
}
